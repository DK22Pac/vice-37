...
int32fromreal(RwReal x)
{
    ...
    // Store the real FPU mode.
    savemode = _controlfp( 0, 0 );
    workmode = savemode;

    // Enable truncate mode.
    workmode |= 0xC;

    // Load new floating point control.
    _controlfp( workmode, workmode );

    res = (RwInt32)x;

    // Restore fp control.
    _controlfp( savemode, savemode );
