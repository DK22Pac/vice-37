...
RWEXPORT RwPluginRegistry cameraTKList;
...
extern              "C"
{
    ...
    /* Rendering */
RWEXPORT RwCamera* RWAPI RwCameraBeginUpdate(RwCamera * camera);
RWEXPORT RwCamera* RWAPI RwCameraEndUpdate(RwCamera * camera);

RWEXPORT RwCamera* RWAPI RwCameraClear(RwCamera * camera, RwRGBA * colour,
                                  RwInt32 clearMode);

/* Displaying results */
RWEXPORT RwCamera* RWAPI RwCameraShowRaster(RwCamera * camera, void *pDev,
                                       RwUInt32 flags);

/* Creation and destruction */
RWEXPORT void RWAPI RwCameraSetFreeListCreateParams( RwInt32 blockSize,
                                            RwInt32 numBlocksToPrealloc );
RWEXPORT RwBool RWAPI RwCameraDestroy(RwCamera * camera);
RWEXPORT RwCamera* RWAPI RwCameraCreate(void);
RWEXPORT RwCamera* RWAPI RwCameraClone(RwCamera * camera);

/* Offset */
RWEXPORT RwCamera* RWAPI RwCameraSetViewOffset(RwCamera *camera,
                                          const RwV2d *offset);

/* View window */
RWEXPORT RwCamera* RWAPI RwCameraSetViewWindow(RwCamera *camera,
                                          const RwV2d *viewWindow);

/* Projection */
RWEXPORT RwCamera* RWAPI RwCameraSetProjection(RwCamera *camera,
                                          RwCameraProjection projection);

/* Clip planes */
RWEXPORT RwCamera* RWAPI RwCameraSetNearClipPlane(RwCamera *camera, RwReal nearClip);
RWEXPORT RwCamera* RWAPI RwCameraSetFarClipPlane(RwCamera *camera, RwReal farClip);

/* Attaching toolkits */
RWEXPORT RwInt32 RWAPI RwCameraRegisterPlugin(RwInt32 size,
                                           RwUInt32 pluginID,
...
RWEXPORT RwInt32 RWAPI RwCameraGetPluginOffset(RwUInt32 pluginID);
RWEXPORT RwBool RWAPI RwCameraValidatePlugins(const RwCamera * camera);

/* Frustum testing */
RWEXPORT RwFrustumTestResult RWAPI RwCameraFrustumTestSphere(const RwCamera *
                                                     camera,
                                                     const RwSphere *
                                                     sphere);
...
/* Offset */
RWEXPORT const RwV2d* RWAPI RwCameraGetViewOffset(const RwCamera *camera);

/* Rasters */
RWEXPORT RwCamera* RWAPI RwCameraSetRaster(RwCamera *camera, RwRaster *raster);
RWEXPORT RwRaster* RWAPI RwCameraGetRaster(const RwCamera *camera);
RWEXPORT RwCamera* RWAPI RwCameraSetZRaster(RwCamera *camera, RwRaster *zRaster);
RWEXPORT RwRaster* RWAPI RwCameraGetZRaster(const RwCamera *camera);

/* Clip planes */
RWEXPORT RwReal RWAPI RwCameraGetNearClipPlane(const RwCamera *camera);
RWEXPORT RwReal RWAPI RwCameraGetFarClipPlane(const RwCamera *camera);
RWEXPORT RwCamera* RWAPI RwCameraSetFogDistance(RwCamera *camera, RwReal fogDistance);
RWEXPORT RwReal RWAPI RwCameraGetFogDistance(const RwCamera *camera);

RWEXPORT RwCamera* RWAPI RwCameraGetCurrentCamera(void);

/* Projection */
RWEXPORT RwCameraProjection RWAPI RwCameraGetProjection(const RwCamera *camera);

/* View window */
extern const RwV2d* RWAPI RwCameraGetViewWindow(const RwCamera *camera);

RWEXPORT RwMatrix* RWAPI RwCameraGetViewMatrix(RwCamera *camera);

/* Frames */
RWEXPORT RwCamera* RWAPI RwCameraSetFrame(RwCamera *camera, RwFrame *frame);
RWEXPORT RwFrame* RWAPI RwCameraGetFrame(const RwCamera *camera);
#endif /* (defined(RWDEBUG) || defined(RWSUPPRESSINLINE)) */

/* RWPUBLICEND */

/* Open closing */
RWEXPORT void* RWAPI _rwCameraClose(void *instance, RwInt32 offset,
                                   RwInt32 size);
RWEXPORT void* RWAPI _rwCameraOpen(void *instance, RwInt32 offset,
                                  RwInt32 size);
