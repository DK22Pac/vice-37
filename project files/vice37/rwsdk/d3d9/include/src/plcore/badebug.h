...
RWEXPORT RwDebugHandler RWAPI RwDebugSetHandler(RwDebugHandler handler);
RWEXPORT void RWAPI RwDebugSetTraceState(RwBool state);

RWEXPORT void RWAPI _rwDebugSendMessage(RwDebugType type,
                                        const RwChar * file,
                                        const RwInt32 line,
...
/* Sending a message */
RWEXPORT RwChar* RWAPI _rwdberrcommon(RwInt32 code, ...);

#if (!defined(DOXYGEN))
/* Doxy doesn't appear to like the __RWFORMAT__ attribute */
RWEXPORT RwChar* RWAPI _rwdbsprintf(const RwChar * format,
                                 ...) __RWFORMAT__(printf, 1, 2);
#endif /* (!defined(DOXYGEN)) */
/* RWPUBLICEND */

/* Opening and closing */
RWEXPORT RwBool RWAPI _rwDebugOpen(void);
RWEXPORT RwBool RWAPI _rwDebugClose(void);

/* Setting Stream handlers at start */
RWEXPORT RwBool RWAPI _rwDebugStart(void);
