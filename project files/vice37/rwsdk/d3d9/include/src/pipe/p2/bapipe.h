...
RWEXPORT RwInt32      _rxPipelineGlobalsOffset;
...
RWEXPORT void* RWAPI _rwRenderPipelineOpen( void *instance, RwInt32 offset, RwInt32 size);
RWEXPORT void* RWAPI _rwRenderPipelineClose(void *instance, RwInt32 offset, RwInt32 size);

/* API */

RWEXPORT RwBool RWAPI _rwPipeAttach(void);

#define rwPipeAttach() _rwPipeAttach()

RWEXPORT void RWAPI _rwPipeInitForCamera(const RwCamera *camera);
#define rwPipeInitForCamera(_camera)  _rwPipeInitForCamera(_camera)

#if (defined(RWSEATIDENTIFIER))
RWEXPORT RwBool RWAPI _rwSeatIdentifier(RwChar *RootPathName,
